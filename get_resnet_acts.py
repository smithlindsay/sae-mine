...
traindata = torchvision.datasets.ImageNet(root="/scratch/gpfs/DATASETS/imagenet/ilsvrc_2012_classification_localization",
...
def get_act(layer=1):    
    ...
    output_file = h5py.File(f"./data/act_{layer}conv{conv_num}.hdf5", "w")
    output_file.create_dataset('cen', (len(traindata), ch), dtype="f")
    output_file.create_dataset('all', (len(traindata), (n*n)), dtype="f")
